...
public class ItemProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        // Create the ItemDbHelper object to interact with the database.
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        // Figure out of the URI matcher can match the URI to a specific code
        int match = sUriMatcher.match(uri);
        switch (match) {
            case ITEMS:
                /**
                 * For the ITEMS code, query the items table directly with the given projection,
                 * selection, selection arguments, and sort order. The cursor could contain
                 * multiple rows of the items table.
                 */
            ...
            case ITEM_ID:
                /**
                 * for the ITEM_ID code, extract out the ID from the URI. For example URI such as
                 * "content://sigildesigns.inventory/items/3", the selection will be "_id=?" and the
                 * selection argument will be a String array containing the actual ID of 3 in
                 * this case.
    ...
    // Helper method to insert an item into the database with the given ContentValues. Returns the
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        // Check that quantity is not null, and at least 0
        Integer qty = values.getAsInteger(ItemContract.ItemEntry.COLUMN_ITEM_QTY);
        if (qty != null && qty < 0) {
            throw new IllegalArgumentException("Item requires a valid quantity.");
        }
        // Check that the price is not null, and at least 0
    ...
    // Updates the data at the given selection and selectionArgs with the new ContentValues.
    @Override
    public int update(Uri uri, ContentValues contentValues, String selection, String[]
        ...
        switch (match) {
            ...
            case ITEM_ID:
                // For the ITEM_ID code, extract out the ID from the URI, so we know which row to
    ...
    // Helper method to update pets in the database with the given content values. Apply the changes
    ...
    private int updatePet(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // Check that the name is being updated
        ...
        // Check that the quantity is being updated
        if (values.containsKey(ItemContract.ItemEntry.COLUMN_ITEM_QTY)) {
            // Check that the qty is not null, and is at least 0
            Integer qty = values.getAsInteger(ItemContract.ItemEntry.COLUMN_ITEM_QTY);
            if (qty != null && qty < 0) {
                throw new IllegalArgumentException("Item requires a valid quantity.");
            }
        }
        // Check that the price is being updated
        ...
        // No need to check the description, or picture.
        ...
        int rowsUpdated = database.update(ItemContract.ItemEntry.TABLE_NAME, values, selection,
                selectionArgs);
